...
if (($handle = fopen("tumor_output.csv", "r")) !== FALSE) {
    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
        ...
        if ($patientNo <> $patientFile->key() + 1 && $row > 0) {
            // for ($i = 0; $i < $patientFile->key(); $i++) {
            // var_dump("CURRENT: " . $data[0]);
            // var_dump("PREVIOUS: " . $tumoursData[$row - 1][0]);
            // var_dump("----------------------------------------------------------------");

            // var_dump("CURRENT: " . $data[17]);
            // var_dump("PREVIOUS: " . $tumoursData[$row - 1][17]);
            // var_dump("----------------------------------------------------------------");
